...
-- 4. Basic RLS Policies (Allow all authenticated users to read/write for now)
-- You can restrict these further based on auth.uid() or profiles.role later.
...
CREATE POLICY "Allow authenticated users to read project_assignments" ON project_assignments FOR SELECT TO authenticated USING (true);
CREATE POLICY "Allow authenticated users to insert project_assignments" ON project_assignments FOR INSERT TO authenticated WITH CHECK (true);
CREATE POLICY "Allow authenticated users to update project_assignments" ON project_assignments FOR UPDATE TO authenticated USING (true);
CREATE POLICY "Allow authenticated users to delete project_assignments" ON project_assignments FOR DELETE TO authenticated USING (true);

CREATE POLICY "Allow authenticated users to read contracts" ON contracts FOR SELECT TO authenticated USING (true);
